...
regd_users.delete("/auth/review/:isbn", (req, res) => {
  ...
  const isbn = req.params.isbn;
  // const filtered_review = 
  if (booksByISBN[isbn]) {
    let existed_Review = booksByISBN[isbn].review;
    if (existed_Review[username]) {
     const deleted_review =  Object.values(existed_Review[username]).filter(u => u.username != username);
      return res.status(200).json({ message: "Review deleted successfully" })
    } else {
      return res.status(400).json({ message: "Unable to delete review" })
    }
  } else {
    return res.status(404).json({ message: "Book by this isbn number isnot present" })
  }
